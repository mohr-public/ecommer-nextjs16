...
function NavItems({ isModalView = false, isAdminView, router }) {
  return (
    <div className={`items-centre justify-between w-full md:flex md:w-auto ${isModalView ? "" : "hidden"}`} id="nav-items">
...
export default function Navbar() {
  ...
  useEffect(() => {
    if (pathName !== null && pathName !== '/admin-view/add-product' && currentUpdatedProduct !== null) {
      console.log('reset currentUpdatedProduct');
  ...
  // const isAdminView2 = pathName.startsWith('/admin-view');
        ...
        <div className='max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4'>
          ...
          <div className='flex md:order-2 gap-2'>
            ...
            }
            {
              user?.role === 'admin' ? (
                isAdminView ? (
                  <button className={"mt-1.5 inline-block bg-black px-5 py-3 text-xs font-medium uppercase tracking-wide text-white cursor-pointer"}
                  onClick={() => router.push('/')}
                  >
                    Client View
                  </button>
            ...
            {
              isAuthUser ? (
                <button onClick={handleLogout} className={"mt-1.5 inline-block bg-black px-5 py-3 text-xs font-medium uppercase tracking-wide text-white cursor-pointer"}>Logout</button>
              ) : (
                <button onClick={()=> router.push('/login')} className={"mt-1.5 inline-block bg-black px-5 py-3 text-xs font-medium uppercase tracking-wide text-white cursor-pointer"}>Login</button>
              )
            }
            <button
